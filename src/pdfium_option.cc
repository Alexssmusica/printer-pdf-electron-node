...
std::unique_ptr<PdfiumOption> V8OptionToStruct(const Napi::Value &options)
{
    ...
    if (options.IsObject())
    {
        ...
        if (obj.Has("paperSize")) {
            ...
            if (paperSize == "A4") ops->paperSize = PaperSize::A4;
            else if (paperSize == "LETTER") ops->paperSize = PaperSize::LETTER;
            else if (paperSize == "LEGAL") ops->paperSize = PaperSize::LEGAL;
            else if (paperSize == "A3") ops->paperSize = PaperSize::A3;
            else ops->paperSize = PaperSize::CUSTOM;
        }

        // Process margins
        if (obj.Has("margins")) {
            auto margins = obj.Get("margins").ToObject();
            if (margins.Has("top")) ops->margins.top = margins.Get("top").ToNumber().FloatValue();
            if (margins.Has("right")) ops->margins.right = margins.Get("right").ToNumber().FloatValue();
            if (margins.Has("bottom")) ops->margins.bottom = margins.Get("bottom").ToNumber().FloatValue();
            if (margins.Has("left")) ops->margins.left = margins.Get("left").ToNumber().FloatValue();
        }
